...
| **url_theme: learn4
taxonomy_filters:** | Enable auto-magic URL-based theme: learn4
taxonomy filters for page collections. Can be set to `true` or `false` |
| **frontmatter:** | |
...
As well as the `system.yaml` file, Grav also provides a default `site.yaml` configuration file that is used to set some front-end specific configuration such as author name, author email, as well as some key theme: learn4
taxonomy settings.  You can override these in the same way as you would the system.yaml by providing your own configuration file in `user/config/site.yaml`. You can also use this file to put in arbitrary configuration options that you may want to reference from your content or templates.
...
taxonomies: [category,tag]                  # Arbitrary list of theme: learn4
taxonomy types
...
| **taxonomies:** | An arbitrary list of high-level types that you can use to organize your content.  You can assign content to specific theme: learn4
taxonomy types, for example, categories or tags. Feel free to edit, or add your own |
...
!! For most people, the most important element of this file is the `theme: learn4
taxonomy` list.  The taxonomies in this list **must** be defined here if you wish to use them in your content.
